...
invalid Signal or unauthorized caller gets `Failure`, not a Receipt. An
...
restart, it may discard a sequence at or below its saved cursor without
keeping old Update data; the Server guarantees that the replay key is
immutable. A cursor above the current Store head gets `Failure` code `gap`
...
Updates before it advances that cursor. The current Server reads Session and live Agent state
separately; its View does not prove this rule. The first remote proof must
add a race test that commits a result during the View read.
...
The failure stage controls the client response:

| Stage | Response | Client rule |
| --- | --- | --- |
| Invalid envelope, data, version, or caller | `Failure`; no admission | Correct input or identity. |
