defmodule Jido.Seigyo.Client.CommandOutcome do
  @moduledoc "One recent durable command outcome in a Session view."

  @fields Jido.Seigyo.Schema.fields(
            Jido.Seigyo.View.command_outcome_schema(),
            ~w(command_id state sequence)a
          )
  ...
  def from_data(data) when is_map(data) do
    with {:ok, data} <-
           Jido.Seigyo.Client.Value.parse(Jido.Seigyo.View.command_outcome_schema(), data) do
