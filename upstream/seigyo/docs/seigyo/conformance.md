...
## Opt-in initialization proof

The [initialization rules](initialization.md) extend the join under the approved
compatibility decision. They do not change the frozen coding v1 bundle.

| Requirement | Independent assertion | Supporting unit proof |
| --- | --- | --- |
| SEIGYO-INIT-001 | `initialization_test.exs`: selected digest, features, limits, and Session open | `jido_seigyo/test/initialization_test.exs`: deterministic selection and published schema |
| SEIGYO-INIT-002 | Raw unsupported required feature fails; a later valid offer succeeds | No common version; unsupported profile/version; bounded malformed offers |
| SEIGYO-INIT-003 | Raw pre-join call, duplicate opt-in join, preserved first channel, and legacy replacement | Phoenix transport marker is connection-local |
| SEIGYO-INIT-004 | Typed and raw clients cannot attach unselected Progress | Unknown optional features are omitted; required features are mandatory |
| SEIGYO-INIT-005 | A supported read by another principal returns `not_found` | Duplicate static descriptors and reserved feature names fail |
| SEIGYO-INIT-006 | `client_message_test.exs`: a controlled peer sends an unoffered selection; client startup fails | Malformed selections, digests, limits, and capability identity fail |
| SEIGYO-INIT-007 | Reconnect and attach to an upgraded Session fail for missing saved requirements; another principal still gets `not_found` | Session map, old JSON journal, and old binary journal migration retain the frozen identity |
| SEIGYO-INIT-008 | `echo_profile_test.exs`: a non-coding static fixture completes open, submit, replay, Result, duplicate retry, and reconnect; coding operations fail | Owner-qualified feature names and the reserved `seigyo.*` namespace |

These acceptance files are in `apps/jido_code_acceptance/test`. The echo
fixture tests the profile boundary. It is not advertised by the production
server and does not claim durable storage after the fixture stops. PostgreSQL
integration cases require `JIDO_CODE_DATABASE_URL`; skipped cases remain a
validation gap.

## Replay and application proof

[Replay](replay.md) defines the S06 corrections without a new wire grammar.

| Requirement | Independent boundary assertions | Reference proof |
| --- | --- | --- |
| SEIGYO-REPLAY-001 | `seigyo_test_007_update_replay_test.exs`, `seigyo_replay_recovery_test.exs`: saved sequence, publication loss, and recovery | Session transitions save facts before publication |
| SEIGYO-REPLAY-002 | `client_replay_test.exs`: equal overlap and conflicting applied, in-flight, and queued data | Subscription replay cut and `ReplayTest` fingerprint comparisons |
| SEIGYO-REPLAY-003 | Controlled wrong-Session page and wrong-identity/invalid Result; existing unreadable event and raw schema cases | Closed Update/Result/page validators |
| SEIGYO-REPLAY-004 | Old delivery after cursor-only restore causes resync | Bounded evidence and missing-fingerprint vectors |
| SEIGYO-REPLAY-005 | Replay-page acknowledgement, partial apply, changed/out-of-order acknowledgement, dead receiver, and bounded local overflow | Pure check leaves cursor unchanged |
| SEIGYO-REPLAY-006 | Concurrent and failed replacement, incomplete page, live attach race, server overflow, connection replacement | Subscription gap and conflict pause; receiver monitor tests |
| SEIGYO-REPLAY-007 | Commit followed by failed publication, execution diagnostics lost, saved Result unchanged; Progress does not move cursor | Trace is not used to settle the client |
| SEIGYO-REPLAY-008 | Two readers with different cursors finish at one fixed View cut while another Command is active | Page request limit is bounded by the fixed target |
| SEIGYO-REPLAY-009 | Controlled socket delivery and replay produce the same Update values | Six published JSON vectors and page-cut/partial-apply model checks |

The named acceptance files are in `apps/jido_code_acceptance/test`, with
journey files in `scenarios/`. `jido_seigyo/test/replay_test.exs` tests the pure
reference reducer. `jido_code_server/test/live_updates_subscription_test.exs`
tests the native stream boundary. Trace completeness is separate S08 work;
these tests prove that absent diagnostics cannot remove a saved Result.

## Session lifetime proof

[Session lifetime](session-lifetime.md) defines configuration binding,
revision domains, runtime availability, model provenance, and fork rules.

| Requirement | Boundary evidence |
| --- | --- |
| SEIGYO-SESSION-001 | `coding_session_007_configuration_test.exs`, `coding_session_007_agent_context_test.exs`, execution and tool-profile validation cases |
| SEIGYO-SESSION-002 | `seigyo_session_lifetime_test.exs`: competing writers, lost reply, later revision, current authorization, and stopped execution-placement retry; `seigyo_test_019_mutation_replay_test.exs`: durable retry after restart |
| SEIGYO-SESSION-003 | Queued work crosses a model change with its original configuration; later work uses the new revision |
| SEIGYO-SESSION-004 | Configuration/history survive server replacement; `seigyo_replay_recovery_test.exs` retains Results after execution-owner loss; long Session and recovery cases |
| SEIGYO-SESSION-005 | `seigyo_test_002_effective_model_test.exs`, configuration and normalized-Result cases bind saved model and digests independently from retry input |
| SEIGYO-SESSION-006 | `coding_session_009_context_compaction_test.exs`: immutable History, ordered compaction boundaries, and Result context revisions |
| SEIGYO-SESSION-007 | Fork commit barrier raced by a configuration Update; inherited required features; `coding_session_010_fork_side_chat_test.exs` and `coding_session_017_current_head_fork_test.exs`: lineage, current cut, isolation, shared read-only policy, and durable retry |

These files are in `apps/jido_code_acceptance/test/scenarios`. Scenario
assertions use protocol values. The fixture controls only runtime conditions
and barriers. PostgreSQL checks remain subject to the stated database-test
prerequisite.

## S08 output evidence

| Requirement | Independent behavior evidence and limits |
| --- | --- |
| SEIGYO-OUTPUT-001 | `seigyo_output_limits_test.exs`: saved Result, later Workspace change, restart, cross-owner reads |
| SEIGYO-OUTPUT-002 | `coding_session_011_attachments_test.exs`: declaration, chunk order, interrupted upload, verified commit, wrong-Session use; Artifact output and timed expiry are not advertised |
| SEIGYO-OUTPUT-003 | Published boundary vectors and raw wire checks; `seigyo_output_limits_test.exs` checks 32,768 UTF-8 bytes; the independent Python validator adds S09 custom-rule checks |
| SEIGYO-OUTPUT-004 | `client_limits_test.exs`: pending call slot, timeout release, pending watch budget, 4,100 Progress pushes; `client_socket_limits_test.exs` adds native transport overflow evidence; replay overflow cases cover delivery queues |
| SEIGYO-OUTPUT-005 | Output scenario and existing Workspace patch / long View cases check explicit loss; omitted Result/Trace/patch bytes have no retrieval claim |
| SEIGYO-OUTPUT-006 | `seigyo_replay_recovery_test.exs`: lost diagnostics declare truncation while Result and retry remain exact; native `trace_test.exs` also checks an observed prefix |
| SEIGYO-OUTPUT-007 | Existing principal, Trace redaction, known-failure, and Workspace catalog cases; native Trace tests reject private adapter failure terms |

S08 passed 122 acceptance tests with four concurrent scenarios, plus one
optional SmolBox skip. Runs at concurrency 20 exposed existing fixture failures
in client isolation and the conservative file-home TCP lock. These remain
visible limitations; a passing bounded run is not proof of those conditions.

## S09 independent verification

The [verification guide](verification.md) records all nine work units, test
results, and limits. Its [requirement matrix](verification-matrix.json) maps
70 current requirements to assertions or an explicit exclusion. The matrix
checks evidence coverage; it does not claim full production conformance.

The Python verifier runs outside the repository with copied public artifacts
and the standard library. The external Elixir consumer has separate dependency
and build directories and fails if server modules are available. Both complete
initialization, admission, reconnect, duplicate retry, replay, and Result reads.
The [recorded report](verification-result.json) includes the frozen contract
digest and the tested working-tree revision label.

S09 passed 21 Python tests, 106 protocol tests, 16 Storage tests, 90 Server
tests, and 126 acceptance tests. PostgreSQL cases ran on an isolated local
PostgreSQL 18 cluster; their earlier skips are resolved. One optional live
SmolBox case remains skipped. Acceptance ran at concurrency 4. The previously
reported concurrency 20 fixture failures remain unresolved. Kernel coverage
is 100%; Session coverage is 72.80% and Storage coverage is 64.58%, both below
their 90% gates. The validation report retains these failures.
